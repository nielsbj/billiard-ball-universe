...
\usepackage{calc}
\newcommand\PAGECOUNT{182}                                   % <-- FINAL page count goes here, last
\newlength\spinew \setlength\spinew{0.002252in * \real{\PAGECOUNT}}  % KDP white paper
...
\definecolor{ivory}{HTML}{F4EFE4}\definecolor{ink}{HTML}{1A1A1A}\definecolor{oxblood}{HTML}{6B2E2E}
...
\begin{tikzpicture}[remember picture,overlay,shift={(current page.south west)},x=1in,y=1in]
  ...
  \ifOXRULE \draw[oxblood,line width=1.1pt] (\fC-1.55,9.125-3.05) -- (\fC+1.55,9.125-3.05);
  ...
  \ifARTFINAL
    \node[anchor=north] at (\fC,9.125-3.28) {\includegraphics[width=\ARTW]{cover_recursive_ball}};
  \else
  ...
  \fill[white] (6.125-0.35-2.0,0.125+0.35) rectangle (6.125-0.35,0.125+0.35+1.2); % KDP barcode zone
\end{tikzpicture}
\end{document}
