...
*For code-claude. Author-commissioned (Niels asked for this section; chat-claude drafted it; Niels approves the draft going into the repo for his read-through). Ships with a companion file `SECTION_collapse.tex` containing the section body — do not retype it. Build-validated on chat-claude's side against the current repo state: both editions, 0 errors, 0 overfull, 153 pages (was 145).*
...
## Patch 1 — insert the section

Insert the **entire contents of `SECTION_collapse.tex`** immediately before this line, which occurs exactly once:

```
\section{What That Teaches}
```

Placement rationale: Chapter 7 then runs five wounds → State of the Wounds → the entropy counter-invoice → this section → "What That Teaches" as the closing. Both constructive sections sit together before the chapter draws its lessons.
...
**The saturation problem is not promoted to a Sixth Wound, and `tab:wounds` is unchanged.** This is Niels's open decision. The section states the problem at full strength and says explicitly that it may belong in the table or may be the sharpest form of the first wound's transparency demand. If Niels later rules "promote it," that is a separate small patch: one table row, one research-program item, and one sentence change in the section. Do not anticipate it.
