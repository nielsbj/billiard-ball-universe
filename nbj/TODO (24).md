...
- **Verify the self-checking figure claim before print [NIELS, 5 minutes].** The making-of note says the three-dimensional figures are made by a program that "refuses to produce the picture at all if a claim in the caption stops being true." In the proofviz studio, deliberately break one `check.equal` in a sketch and run `node scripts/make-deliverables.mjs bbu_<name>`. Confirm it fails rather than exporting. If it exports anyway, tell chat-claude and the sentence retreats to the already-true "drawn in code, so that the geometry could be checked rather than eyeballed." This is the one chapter whose whole value is being exactly right about how the book was made.
