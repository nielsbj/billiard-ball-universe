...
Six geometric figures are now rendered PLATES from Niels's proofviz studio (JS sketches → cropped vector PDFs in `figures/`, included via `\includegraphics`): inversesquare, shadowpush, layers, actorsbelow, circulation, lightpair. Rules: (1) the `.sketch.js` is the single source of truth — edit it, never the PDF; regenerate via `node scripts/make-deliverables.mjs bbu_<name>` + pdfcrop (workflow comment sits above each figure in the .tex). (2) **Re-corrected 2026-07-27, second pass — the studio then made the strong version true.** All six 3D sketches now carry falsifiable `check.equal` assertions (the tautological tangent check was replaced by a real perpendicularity test; `layers` and `actorsbelow` gained cross-panel magnification checks). `make-deliverables --strict` (or `PROOFVIZ_STRICT=1`) exits non-zero and writes **nothing** if any check fails; advisory remains the development default by design. **Final plates for print must be generated under `--strict`** so the book's claim describes the actual files. Superseded note, kept for the lesson: this entry previously said "export fails loudly if a claim breaks", inherited verbatim from a handover note and false at the time ---  — the handover note overstated this and the book repeated it.** Four of the six sketches carry `check.equal` assertions (inverse_square, shadowpush, circulation, lightpair); `layers` and `actorsbelow` carry none. The checks are **advisory, not blocking**: a failed check prints `check ✗` and is recorded in the figure's README, but the export still writes the PDF and exits zero (declared contract in make-deliverables.mjs, interpret.js, figure-lint.js). Treat the checks as ground truth for caption claims — but never assume a bad figure cannot ship. Read the build output. (3) Panel captions are BAKED into the plates for layers/actorsbelow/circulation — never double-caption those panels in LaTeX. (4) New figures: genuinely 3D subject (space, shadows, orbits) → proofviz sketch; text-and-boxes (timelines, labeled curves, flowcharts) → TikZ, with room left around every label. (5) Palette unchanged; addendum: bbuBlue may also serve as the second ink for one of two opposed rotation senses where arrowheads alone are too subtle — the binding rule is grayscale separation, always.
