...
- ✔ **DONE — `fig:saturation` is in the Sixth Wound.** Niels saw the plate and ruled it in; the conditional commission is closed. **One row still open:** the caption was written without the studio's check report, so it deliberately asserts only physics (thin limit → sum over matter; thick limit → outline) and no tolerances. **Reconcile the caption against the sketch's actual `check.equal` assertions before print** — the making-of chapter now claims exactly this relationship holds, so an unreconciled caption would undercut it. Also confirm the two exported filenames match what the figure block includes.
