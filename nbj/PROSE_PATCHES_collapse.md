...
*For code-claude. Author-commissioned (Niels asked for this section; chat-claude drafted it; Niels approves the draft going into the repo for his read-through). Ships with a companion file `SECTION_collapse.tex` containing the section body — do not retype it. Build-validated on chat-claude's side against the current repo state: both editions, 0 errors, 0 overfull, 151 pages (was 145).*
