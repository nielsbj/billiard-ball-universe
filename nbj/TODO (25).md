...
- **Regenerate all six plates with `make-deliverables --strict`** (or `PROOFVIZ_STRICT=1`) and re-copy them into the book's `figures/`. The pictures are byte-identical either way — this is purely so that the making-of chapter's sentence, "the program that made the figures in this book refuses to draw at all if one of those statements has stopped being true," describes the actual files that were printed. Confirm all six exit 0 under strict.
