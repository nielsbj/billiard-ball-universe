...
The second item looked like a second debt and turned out, on inspection, to be something the tower can pay. Black holes are credited with an entropy, and it is famously proportional to the \emph{area} of the horizon rather than to the volume within. The entropy section of this chapter counted arrangements in volumes, layer by layer, so a change of shape at a horizon wants explaining. But recall what that counting was: never a fact about the region itself, always a fact about what the layer above can tell apart. And a horizon is precisely a surface beyond which our layer can tell nothing apart at all. Whatever the interior is doing reaches us only as what the surface expresses; the arrangements our layer can distinguish are therefore surface arrangements, and the count is a surface count. An area law is what layer-indexed entropy \emph{must} give at a censored boundary. It costs nothing and assumes nothing new.

There is a stronger version, and it is the one I reached for first. Let the crowd itself be stopped at the surface --- let a collapsed body genuinely shield, so that all the outside can ever interact with is the shell, the interior having been pressed dense by the shadow of its own shell. That account is mechanical rather than merely bookkeeping, and I find it the more satisfying of the two by some margin. It is also precisely what the sixth wound charges for, now twice over: if the crowd really is stopped at the shell then the gravity comes out wrong, and a shell sized by the body rather than by the horizon grows too slowly to give an area law of the observed shape. So I take the cheap account, which works and costs nothing, and leave the satisfying one standing on the invoice where it can be argued with. Somebody may yet find a way to have it; it will not be had cheaply.
