...
6. **Ship the print plates under `--strict` [NIELS — before the print interior is cut].** Regenerate all six plates with `make-deliverables --strict` and re-copy them into `figures/`. The pictures are byte-identical either way; this is purely so the making-of sentence ("the program that made the figures in this book refuses to draw at all if one of those statements has stopped being true") describes the actual files printed. Confirm all six exit 0 under strict. (Studio Priority 2/3 — a proofviz plate for `fig:dipolequadrupole`, a `checks.json` manifest — are optional and fine to leave undone.)
