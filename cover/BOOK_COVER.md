...
- **Spine width = page count × 0.0025 in** (cream paper). At the final **202 pages: 0.5050 in**. The cover currently carries `\PAGECOUNT{182}`, deliberately: the spine is the last thing set, in the release commit. ⚠ Anything that changes the interior page count changes this — and the constant and the count must be right *together*, or the wrap is wrong twice over.
