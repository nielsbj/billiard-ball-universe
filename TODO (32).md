...
### 3D-collapse — CLOSED (run 4)
All five entries verified with DOIs/issues added; three citations placed (chandrasekhar1931 at the 1.5-M☉ limit, biography confirmed — born 1910, the calculation made on the 1930 India→England voyage, so nineteen checks out; bekenstein1973 at the area-entropy sentence; oppenheimervolkoff1939 + ligo2017 co-cited at the neutron maximum, since O–V's own figure is ~0.7 M☉ for an ideal gas). Two citation *targets* were wrong and were caught before shipping: the Baade–Zwicky neutron-star proposal is in the companion paper, and the EHT sentence is Sgr A* (2022), not M87 (2019). Both fixed by `BIB_PATCH_collapse_targets.md`.

### 3D-dark. The dark-matter section (new, unverified) [CC]
- `mcgaugh2016` — **load-bearing.** The claim that galaxies of very different sizes begin to misbehave at nearly the same *acceleration* rather than the same *radius*. Confirm this fairly states the radial acceleration relation and holds over the implied range of galaxy sizes. **If it overstates, STOP and report** — the section calls this its hardest argument. [CC]
- `clowe2006` — confirm the Bullet Cluster description ("the mass, as measured by the bending of light, has come apart from the visible matter and sits to either side of it"). [CC]
- `rubin1980` — correct paper for flat rotation curves as described; pages. [CC]
- `zwicky1933` — pages and the cluster-velocity inference. Same Zwicky as `baadezwicky1934cr`; flag if the coincidence deserves a footnote. [CC]
- **Screening direction** — confirm that absorption over long paths makes a flux-based gravity *weaker* than inverse-square at range, not stronger. Elementary; check rather than assume. [CC]

### 3D-geometry. The packing-not-resonance passage (new, unverified) [CC]
...
- **The anisotropy bound as worded** — "about a part in a hundred thousand million million million" (i.e. 10⁻²³?) for direction-dependence of light speed. **This must be checked against `herrmann2009`, the 10⁻¹⁷ bound already cited in the second wound.** If the word-form does not match the cited figure, STOP and report — do not silently adjust either. [CC]
...
### Publishing disclosure [NIELS]
- ~~Verify the self-checking figure claim before print~~ **CLOSED 2026-07-27 — tested in the studio, claim was FALSE.** The export prints `check ✗` and writes the PDF anyway (advisory by declared contract); four of six sketches carry checks, not all six; one asserted check is a tautology that cannot fail. The book's sentence retreated to the verified form (v4) and `MANUAL.md` §3b was corrected at source. Studio backlog item, not a book item: fix or delete the tautological tangent check in `bbu_inverse_square`.
- The book now carries "A Note on How This Book Was Made" in the back matter. **Check KDP's current AI-content disclosure requirements before publishing** — the questions asked at upload have changed more than once, and AI-generated cover art is treated differently from AI-assisted text. Answer them consistently with what the note says.
- Decide whether to name the model/vendor in the note. Currently unnamed by recommendation (product names date a book); it is a one-sentence addition if wanted, and should be the author's own sentence.
- Decide where the repository URL lives — copyright page or landing page, not the note itself, so it can be corrected without a reprint.

### Figure provenance [NIELS — before the print interior is cut]
- **Verify, do not regenerate.** Ask the studio to run `make-deliverables --all --strict` and report the exit codes; six clean exits is the evidence the making-of chapter's sentence needs. **Do not copy new plate PDFs into the book.** The studio has already proven the drawings are byte-identical before and after its changes, so the figures printed here are the figures that pass the gate — and regenerated PDFs would differ in metadata while being identical in ink, buying a diff, a rebuild and a pagination risk for no visual gain. Keep the strict-run output with the project record.
- If any plate ever *does* fail strict in future, that is the moment to regenerate and re-copy — and the book's sentence would then need revisiting before print.
- Optional, Niels's call: the studio's Priority 2 (a proofviz plate for `fig:dipolequadrupole`) and Priority 3 (a `checks.json` manifest the book's build could verify) are both unstarted and both fine to leave undone.

### Figures — one conditional commission [NIELS decides after the read-through]
- ✔ **DONE — `fig:saturation` is in the Sixth Wound.** Niels saw the plate and ruled it in; the conditional commission is closed. **Row CLOSED:** the studio's check report arrived and the caption was reconciled against all four assertions (thin ∝ N; doubling doubles; thick saturated; trebling barely moves) — the doubling/trebling sentence in the caption exists *because* two checks underwrite it. Filenames confirmed as `bbu_sixthwound_transparent` / `bbu_sixthwound_opaque`. Remaining: the studio generates under `--strict` and delivers the two cropped PDFs into `figures/`.
- ~~`COMMISSION_sixthwound.md` is with the proofviz studio~~: a two-panel plate for the Sixth Wound showing transparent (rays stopped by scattered individual absorbers, nothing in shadow → push tracks mass) versus opaque (rays stopped at the front face, interior in shadow → push tracks silhouette). It is commissioned in parallel with your reading and **inserted only if the cold read says the wound needs it**. Costs nothing if you decline.
- Figure map for reference: 23 figures/tables, nineteen of them in Chapters 6–7, and none in Chapter 5 — the distribution already follows "pictures where geometry does the work". The Sixth Wound is the one genuinely spatial argument still carried by prose alone. Second-order candidates, only if the read-through calls for them: a small locality diagram for Chapter 6's Second Rule (TikZ, cheap), and a clearly-schematic rotation curve for the dark-matter section (careful — a schematic that reads as data would be wrong in this book).
- Standing source rule: OpenAI images for chapter openers only, never for an argument; TikZ for text-and-boxes; proofviz for anything three-dimensional or where the geometry *is* the claim.

### Standing fragility — `fig:inversesquare` [check before any edit to it]
- It renders at roughly **500pt of a 532pt text block: 94% of the page**, with about **32pt of slack — two and a half caption lines**. It does not overflow today and needs no change today. But **any edit that lengthens its caption, or any change to caption font or leading, must be followed by a build check for `Overfull \vbox` specifically.** This is the same class of error that put `fig:saturation` four lines off the bottom, and it is the only remaining float in the book with less than half an inch of headroom.
- Do **not** pre-emptively shrink it. It is correct as it stands, and late churn is the greater risk.

### ⚠ BLOCKER — which tree builds the print interior? [settle before Phase 6]
- The proofviz studio has flagged twice that the `tex/` mirror in the outer `Everything` repo differs from `code-claude/`: it is missing the corrected inverse-square plates, the far-panel width change, and the `bbu_sixthwound` plates. **Two trees can produce a PDF, they are known to differ, and no rule says which is real.**
- Required: (a) establish in writing which tree `build.bat` runs in for the print interior, and whether `tex/` has its own `figures/`; (b) make the non-build tree *impossible* to mistake — delete, rename, or drop a `DO_NOT_BUILD_HERE.txt` beside its `.tex`; (c) record the answer in `FILE_OWNERSHIP.md`, which governs which file is master and should now govern which tree is.
- Urgency: everything upstream is finished. Draft notes off → index in → **build print interior** → page count → spine → wrap → proof. If this is discovered at the proof stage, the proof is wasted and the spine is wrong.

### fig:inversesquare width — check on the physical proof [NIELS]
- Far panel is now `0.859\textwidth` against the near panel's `0.9`, chosen so the pair's on-page height is unchanged to within 0.05pt and the ~32pt slack is fully preserved. Correct call on the axis that broke before.
- Cosmetic consequence: the panels differ in width by 14pt — 0.09 in of extra white each side of the far panel. Their aspects already differ hugely (0.93 vs 0.51), so it probably will not read as an error, **but look at it on paper.** If it does, both at `0.9` costs 7.2pt of the 32pt slack (leaving ~25pt, about two caption lines) and is also safe.
